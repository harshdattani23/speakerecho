...
import { AppStatus, AudioData, TranscriptionResponse, Emotion } from './types';
...
function App() {
  ...
  const handleLiveFinished = () => {
    if (liveTranscript) {
        setResult({
            summary: "Real-time transcription session.",
            segments: [{
                speaker: "User",
                timestamp: "Live",
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
            Upload a file, record directly, or use <span className="text-indigo-600 dark:text-indigo-400 font-semibold">Real-time Transcribe</span> to see text as you speak.
          </p>
        </div>
          ...
          {!result && status !== 'processing' && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 sm:p-8 transition-colors duration-300">
              {mode === 'record' ? (
                ...
                <RealTimeRecorder 
                    onTranscriptionUpdate={(text) => setLiveTranscript(prev => prev + " " + text)} 
